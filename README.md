...
| **개인화 피드** | 관심사 기반 정렬 + 5개 카테고리 탭 (전체/정치/경제/시사·사회/과학기술/스포츠·연예) + 스테일-화이트-리밸리데이트 캐시 |
| **사이드바 위젯** | 속보 20건 · 시세 20개 · 인기 기사 20건 · 날씨(Open-Meteo) |
| **검색** | 앱내 즉시 필터 + Google News RSS 검색 · 최근 검색 기록 저장 |
| **기사 상세** | 점진적 로딩 — 편향 바(즉시) → AI 요약·분석(Agent A) → 다른 논조 추천 |
| **AI 정밀 분석** | "AI 정밀 분석" 버튼 → Agent C가 반대 성향 기사 선별·이유 설명 |
...
| 에이전트 | 트리거 | 역할 |
|----------|--------|------|
| **Agent A** (분석관) | 기사 상세 진입 시 자동 | 원문 요약 3문장 · 편향 판별 설명 · 찬반 시각 · 배경 정보 · 교차검증 포인트 |
| **Agent C** (관점 탐색기) | "AI 정밀 분析" 버튼 클릭 (수동) | 1차 추천 후보 중 다른 논조 기사 선별 + 추천 이유 |
...
### 1차 논조 추천 (Agent C 없이 무료 자동)

```
기사 제목 → 키워드 추출 (3글자+ 한국어, 일반명사 95개 제외)
→ Google News RSS 재수집
→ 반대 성향 언론사 필터링
   - 보수 계열 원본 → 진보·중립 추천
   - 진보 계열 원본 → 보수·중립 추천
...
| 백엔드 | FastAPI, httpx, feedparser, BeautifulSoup4 |
