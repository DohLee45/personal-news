...
| **AI 정밀 분석** | "AI 정밀 분析" 버튼 → Agent A가 7필드 분析 + 논거 기반 추천 (API 1회) |
| **설정** | 관심 키워드 관리 · 검색 기록 · 닉네임 변경 (3탭) |
| **편향 분析** | UP Score + Shannon 다양성 + 7개 섹션 + 변화 추이 그래프 |
...
S_text   (0.45) — 문장 방향성 분析 + 이중 부정 해소
...
키워드 검색으로 수집된 기사는 제목 기반 간단 분類(`simple_classify`)로 카테고리를 자동 할당합니다.

### 기사 상세 페이지 구조

기사 진입 시 API 호출 없이 즉시 표시되며, AI 분析은 사용자 선택 시에만 실행됩니다.
...
**버튼 클릭 시 (API 1회)**  
④ AI 정밀 분析 — 편향 판별 · 찬반 입장 · 중도 시각 · 맥락 · AI 점수 · 교차검증 · 논거 기반 추천 (5~15초)

원문 요약은 역피라미드 뉴스 구조를 활용한 본문 앞부분 추출이며, AI가 아닌 자체 처리입니다.

### UP (Unbalanced Perspective) Score

사용자의 Stage 2 분析 완료 기사 열람 이력을 분析합니다.
...
| 에이전트 | 트리거 | 역할 |
|----------|--------|------|
| **Agent A** (분析관) | "AI 정밀 분析" 버튼 클릭 (수동) | 편향 판별 · 찬반 입장 · 중도 시각 · 맥락 정보 · AI 편향도 점수 · 교차검증 · 논거 기반 추천 (API 1회) |
...
| `pn_analysis_history` | UP Score 분析 이력 | 30건 |
